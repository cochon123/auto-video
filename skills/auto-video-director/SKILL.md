...
```bash
python3 ~/.config/auto-video/helpers/fetch_media.py \
...
```bash
python3 ~/.config/auto-video/helpers/tts_timestamps.py \
...
[Scenarist Skill] ──► produces scenario JSON with phrase_groups + visual plans + asset queries
    │
    ▼
[fetch_media.py] ──► downloads/generates all media assets
    │
    ▼
[tts_generate.py] ──► generates narration audio per scene (OmniVoice 24kHz, or edge/API)
    │                            WARNING: GPU task - runs ALONE
    ▼
[tts_timestamps.py] ──► extracts word-level timing
