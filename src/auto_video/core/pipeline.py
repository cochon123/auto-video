...
class VideoPipeline:
    ...
    def retry_step(self, video_id: str, step: PipelineStep) -> PipelineResult:
        ...
        if step_value == 5:
            try:
                ...
                self._update_artifacts(state, workspace)
                self._save_state(state)
                logger.info("Subtitles regenerated and burned: %s", workspace.subtitles_path)
        ...
        if step_value == 6:
            try:
                ...
                self._update_artifacts(state, workspace)
                self._save_state(state)
                logger.info("Thumbnail regenerated: %s", workspace.thumbnail_path)
    ...
    def run(
        ...
        workspace = Workspace(self.config.storage.temp_path)
        workspace.create()
        self._workspace = workspace

        if duration is None:
            duration = 180 if format == "long" else 60
        ...
        if self._progress_display:
            ...
            self._progress_display.update_global_progress(0)
        ...
        try:
            ...
            llm = LLM(self.config.llm)
            self._progress = PipelineProgress(workspace.video_id, PipelineStep.SCRIPT, 0.0)

            script = llm.generate_script(title, duration, lang)
            workspace.script_path.write_text(script, encoding="utf-8")
            completed_steps.append(PipelineStep.SCRIPT)
            state.completed_steps.append(1)
            state.current_step = 2
            ...
            if self._progress_display:
                self._progress_display.update_script_content(script)
                self._progress_display.update_artifact_path(0, str(workspace.script_path))
                self._progress_display.complete_step(0, f"Script généré ({len(script)} caractères)")
        ...
        try:
            ...
            tts = TTS(self.config.tts)
            self._progress = PipelineProgress(workspace.video_id, PipelineStep.AUDIO, 0.0)
            ...
            if self._progress_display:
                self._progress_display.update_artifact_path(1, str(workspace.audio_path))
                self._progress_display.complete_step(1, f"Audio généré ({audio_duration:.1f}s)")
        ...
        try:
            ...
            llm = LLM(self.config.llm)
            self._progress = PipelineProgress(workspace.video_id, PipelineStep.VISUALS, 0.0)

            keywords = llm.extract_keywords(script)

            stock_manager = StockManager(self.config.visuals)

            if self.config.visuals.visual_llm and self.config.visuals.mode in ("stock", "hybrid"):
                try:
                    logger.info("Using VisualKeywordExtractor for segment-based clips")
                    keyword_extractor = VisualKeywordExtractor(self.config.visuals.visual_llm)
                    segments_with_keywords = keyword_extractor.extract_keywords_per_segment(script)
                    clips = stock_manager.get_clips_for_segments(segments_with_keywords, clips_dir)
        ...
        try:
            ...
            if mock_detected:
                raise ValueError(
                    ...
                    "  2. Enable 'pexels' and/or 'pixabay' and enter your API keys\n"
                    "  3. Or configure a local assets folder with your own videos\n"
                    f"  4. Then run: auto-video resume --video-id {workspace.video_id}"
                )
            ...
            self._progress = PipelineProgress(workspace.video_id, PipelineStep.MONTAGE, 0.0)

            composer.concatenate_clips(clips, workspace.video_raw_path, audio_duration)
            composer.add_audio(workspace.video_raw_path, workspace.audio_path, workspace.final_path)
            composer.apply_format_with_temp(workspace.final_path, workspace.final_path, format)
