...
class OpenAIProvider(LLMProvider):
    def __init__(self, config: LLMProviderConfig) -> None:
        ...
        self._cache_dir = Path.home() / ".cache" / "auto-video" / "openai"
    ...
    def generate(self, prompt: str, system_prompt: str | None = None) -> str:
        ...
        if cached_content is not None:
            logger.info("Using cached OpenAI response: model=%s", self._model)
        ...
        logger.info("OpenAI API call: model=%s", self._model)
        ...
        try:
            ...
            if content is None:
                raise OpenAIResponseError("Empty response from OpenAI")
    ...
    def generate_with_tokens(self, prompt: str) -> tuple[str, int]:
        ...
        cached_content = self._get_cached_response(cache_key)
        if cached_content is not None:
            logger.info("Using cached OpenAI response with tokens: model=%s", self._model)
            return cached_content, 0

        messages: list[ChatCompletionMessageParam] = [{"role": "user", "content": prompt}]

        logger.info("OpenAI API call with token count: model=%s", self._model)
    ...
    def health_check(self) -> bool:
        try:
            response = self._http_client.get("https://api.openai.com/v1/models")
